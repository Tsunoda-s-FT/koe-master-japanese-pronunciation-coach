...
const App: React.FC = () => {
  ...
  return (
    <AuthGuard>
      <div className="min-h-screen p-4 md:p-8 flex flex-col items-center bg-slate-50 relative">
        
        {/* Provisional Banner */}
        {status === 'provisional' && (
          <div className="w-full max-w-4xl bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl mb-6 flex items-start sm:items-center gap-3 animate-in slide-in-from-top-2">
            ...
            <div className="text-sm flex-1">
              <span className="font-bold">Provisional Account:</span> You have <span className="font-bold">{remainingHours} hours</span> to verify your email. 
        ...
        {user && (
          <div className="absolute top-4 right-4 md:top-8 md:right-8 z-10 flex items-center gap-3">
            <div className="hidden md:block text-right">
              <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Logged in as</div>
              <div className="text-sm font-bold text-slate-700">{user.email}</div>
            </div>
            <button 
              onClick={() => auth.signOut()}
              className="bg-white p-2 rounded-xl shadow-sm border border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-200 transition-colors"
              title="Logout"
            >
              ...
              <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                {state.targetText && (
                  <div className="flex justify-end">
                    <button
                      disabled={isSpeaking}
                      onClick={() => speak(state.targetText, 'ja')}
                      className="text-indigo-600 hover:text-indigo-700 font-bold text-sm flex items-center gap-1 bg-indigo-50 px-3 py-1.5 rounded-xl transition-all disabled:opacity-50"
                    >
                      {isSpeaking ? (
